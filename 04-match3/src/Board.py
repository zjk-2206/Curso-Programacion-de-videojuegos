...
class Board:
    ...
    def render(self, surface: pygame.Surface, dragged_tile: Optional[Tile] = None) -> None:
        for row in self.tiles:
            for tile in row:
                if tile is not None and tile != dragged_tile:
        ...
        for i in range(settings.BOARD_HEIGHT):
            for j in range(settings.BOARD_WIDTH):
                if j < settings.BOARD_WIDTH - 1:
                    t1, t2, = self.tiles[i][j], self.tiles[i][j+1]
                    self.tiles[i][j], self.tiles[i][j+1] = t2, t1
                    t1.i, t1.j, t2.i, t2.j = i, j+1, i, j
                    
                    self.matches = []
                    res = self.calculate_matches_for([t1, t2])
                    
                    self.tiles[i][j], self.tiles[i][j+1] = t1, t2
                    t1.i, t1.j, t2.i, t2.j = i, j, i, j+1
                    self.matches = []
                ...
                if i < settings.BOARD_HEIGHT - 1:
                    t1, t2 = self.tiles[i][j], self.tiles[i+1][j]
                    self.tiles[i][j], self.tiles[i+1][j] = t2, t1
                    t1.i, t1.j, t2.i, t2.j = i+1, j, i, j
                    
                    self.matches = []
                    res = self.calculate_matches_for([t1, t2])
                    
                    self.tiles[i][j], self.tiles[i+1][j] = t1, t2
                    t1.i, t1.j, t2.i, t2.j = i, j, i+1, j
                    self.matches = []
    ...
    def ensure_valid_moves(self) -> None:
